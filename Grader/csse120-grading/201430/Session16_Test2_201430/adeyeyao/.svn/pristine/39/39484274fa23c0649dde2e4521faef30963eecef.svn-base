...
def problem4a(window, circle, rectangle, circle_color, rectangle_color):
    ...
    p4 = zg.Point(p2.x + 2 * (center.x - p2.x), p2.y)
    p5 = zg.Point(p3.x - 2 * (center.x - p2.x), p3.y)
    points_list = [p2, p3, p4, p5]
    for k in range(len(points_list)):
        line1 = zg.Line(p1, points_list[k])
        line1.draw(window)
        line1.setWidth(3)
...
def problem4b(window, n, point, rectangle, colors):
    ...
    for k in range(n):
        ...
        problem4a(window, circle, rectangle1, colors[k], colors[(k + 1) % (n)])
