...
def problem4a(window, circle, rectangle, circle_color, rectangle_color):
    ...
    line3 = zg.Line(center, zg.Point(corner1.x, corner2.y))
    line4 = zg.Line(center, zg.Point(corner2.x, corner1.y))

    for shape in (circle, rectangle, line1, line2, line3, line4):
        shape.setWidth(3)
        shape.draw(window)
...
def problem4b(window, n, point, rectangle, colors):
    ...
    rect = rectangle
    center = point
    radius = abs(rectangle.p1.x - rectangle.p2.x) / 2
    circle = zg.Circle(center, radius)
    shift = 70
    for k in range(n):
        problem4a(window, circle, rect, colors[k % len(colors)],
